...
final class yalstAdmin extends yalstGeneral
{
	...
	/* Hinzufügen des Tabs, wenn keiner im gespeicherten Einbindungscode vorhanden */
	
	private function addTabIfNecessary ($code)
	{
		if (preg_match('/\bscript\.src\s*=\s*.+\.php.+(?<site_entry>site=\d+-\d+).+(?<tab_entry>button=in-page.+?(?:\\x26|\&|$))/im', $code) === 1)
		{
		    return $code;
		}
		
		return preg_replace('/(\bscript\.src\s*=\s*.+\.php.+)(site=\d+-\d+)/im', '\1\2\\x26button=in-page\x26onlinetext=Chat%20online\x26offlinetext=Chat%20offline\x26busytext=Chat%20busy\x26colorback=%232C6060\x26colortext=%23FFFFFF', $code);
	}

	/* Speichern der drei Einbindungscodes und der gewählten Tabauswahl */
	
	protected function update_options($data)
	{
	
		if (isset($data['yalstDeactiveIntegration']) || isset($data['yalstReiterIntegration']) || isset($data['yalstReiterWidgetIntegration']) || isset($data['yalstWidgetIntegration']) || isset($data['yalstTabChatIntegration']))
		{			
			...
			else if (isset($data['yalstTabChatIntegration']))
			{
				$yalstTabChatIntegrationInputCleaned = ini_get('magic_quotes_gpc')
					? stripslashes ($data['yalstTabChatIntegrationInput'])
					: $data['yalstTabChatIntegrationInput'];
				
				$yalstTabChatIntegrationInput = $this->addTabIfNecessary($yalstTabChatIntegrationInputCleaned);
				
				update_option('yalstTabChatIntegrationInput', $yalstTabChatIntegrationInput);
				
				update_option('isAddedTab', strcmp($yalstTabChatIntegrationInput, $yalstTabChatIntegrationInputCleaned) );
			}
